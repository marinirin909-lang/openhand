...
export function TaskCard({ task }: TaskCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "w-full p-[14px] text-left flex items-center justify-between cursor-pointer hover:bg-surface-hover transition-all duration-300 rounded-lg",
    ...
    >
      <div className="flex items-center gap-3 min-w-0 w-full">
        ...
        <div className="flex flex-col gap-1 min-w-0 flex-1">
          ...
          <span
            className="text-xs text-muted leading-4 font-normal max-w-70 truncate"
            title={task.title}
          >
            {task.title}
          </span>
